...
// Strip terminus warning/deprecation noise and slice from the first JSON token.
function cleanJson(raw: string): string {
  const stripped = raw
    .split('\n')
    .filter((l) => !/^\s*(Deprecated|Warning|Notice|PHP)\b/i.test(l))
    .join('\n')
  const start = stripped.search(/[{[]/)
  return start === -1 ? stripped.trim() : stripped.slice(start).trim()
}
