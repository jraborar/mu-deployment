  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 font-mono text-sm text-pantheon-text-muted">Access the deployment console</p>
      </div>
      ...
      <div className="relative">
        <div className="absolute inset-0 flex items-center"><div className="w-full border-t border-pantheon-border" /></div>
        <div className="relative flex justify-center"><span className="bg-pantheon-bg px-3 font-mono text-xs text-pantheon-text-dim">or continue with</span></div>
      </div>

      <div className="space-y-2">
        <button onClick={async () => { const r = await signInWithOAuth('github'); if (r?.url) window.location.href = r.url; if (r?.error) setError(r.error) }} className={oauthCls}>
        ...
        <button onClick={async () => { const r = await signInWithOAuth('google'); if (r?.url) window.location.href = r.url; if (r?.error) setError(r.error) }} className={oauthCls}>
